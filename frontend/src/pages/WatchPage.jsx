...
const WatchPage = () => {
  ...
  if(!content){
    ...
    <div className='bg-black min-h-screen  flex flex-col text-white'>
        ...
        <div className= ' w-full aspect-video mb-8 p-2 sm:px-10 md:px-32 '>
            {trailers.length>0 &&(
        ...
        {/* {movie details} */}
        <div className='flex flex-col md:flex-row items-center jutsify-between gap-20 max-w-6xl mx-auto'>
            <div className='mb-4 md:mb-0'>
            <h2 className='text-5xl font-bold text-balance'> {content?.title || content?.name}</h2> 
            <p className='mt-2 text-lg'>
                {formatReleaseDate(content?.release_date || content?.first_air_date)} ||{" "}{content?.adult ?(
                    <span className='text-red-600'>18+</span>):(<span className='text-green-600'>PG-13</span>)
                }{" "}
            ...
            <div className='mt-12 max-w-5xl mx-auto relative'>
                ...
                <div className='flex overflow-x-scroll scrollbar-hide gap-4 pb-4 group' ref = {slideRef}>
                 ...
                 <ChevronRight
                  ...
                  opacity-0 group-hover:opacity-100 transaction-all duration-300 cursor-pointer
                   bg-red-600 text-white rounded-full' onClick = {scrollRight}/> 

                  <ChevronLeft
                  className='absolute top-1/2 -translate-y-1/2 left-2 w-8 h-8 
                  opacity-0 group-hover:opacity-100 transaction-all duration-300 cursor-pointer
                   bg-red-600 text-white rounded-full' onClick = {scrollLeft}/> 
                    </div>
                    </div>
